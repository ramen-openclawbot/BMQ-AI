...
const ALLOWED_ENDPOINT_HOSTS = new Set(
  (process.env.ALLOWED_ENDPOINT_HOSTS || "api-v2.vietguys.biz")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
);
...
function safeLog(level, message, meta = {}) {
  ...
  for (const key of Object.keys(sanitized)) {
    if (/token|secret|signature|otp|access/i.test(key))
      sanitized[key] = "[redacted]";
  }
  console[level](
    JSON.stringify({ ts: new Date().toISOString(), message, ...sanitized }),
  );
}
...
function sanitizeProviderPreview(text) {
  return String(text || "")
...
async function handleProviderProxy(req, res, routeConfig = {}) {
  let rawBody = "";
  try {
    rawBody = await readBody(req);
  } catch (error) {
    return jsonResponse(res, error.status || 400, {
  ...
  try {
    ...
    safeLog("info", routeConfig.logMessage || "provider response", {
      status: providerResponse.status,
      ok: providerResponse.ok,
      ms: Date.now() - start,
      tracking_id: providerPayload.tracking_id || providerPayload.bid,
      transaction_ids: Array.isArray(providerPayload.transaction_id)
        ? providerPayload.transaction_id
        : undefined,
      response_preview: sanitizeProviderPreview(text),
    });
    ...
    safeLog("error", "provider request failed", {
      aborted,
      ms: Date.now() - start,
    ...
    jsonResponse(res, aborted ? 504 : 502, {
...
const server = http.createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/health") {
    return jsonResponse(res, RELAY_SECRET ? 200 : 503, {
      ok: Boolean(RELAY_SECRET),
      service: "bmq-otp-relay",
    });
  }
  if (req.method === "POST" && req.url === "/send") {
    return handleProviderProxy(req, res, { logMessage: "provider response" });
  }
  if (req.method === "POST" && req.url === "/status") {
    return handleProviderProxy(req, res, {
      defaultEndpoint: "https://api.vietguys.biz:4438/zalo/v1/status",
      logMessage: "provider status response",
    });
  }
