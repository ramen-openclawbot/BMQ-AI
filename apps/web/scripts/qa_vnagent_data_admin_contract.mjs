...
check("generation calls the official DeepSeek OpenAI-compatible endpoint directly", generationClient.includes('https://api.deepseek.com/chat/completions') && generationClient.includes('redirect: "error"') && generationClient.includes("Authorization") && !generationClient.includes("ai-gateway"));
check("generation carries no Gateway-only ZDR payload or retention claim", !generation.includes("zeroDataRetention") && !generationClient.includes("zeroDataRetention") && !generationPricing.includes("zeroDataRetention") && generationPricing.includes('retention: "not asserted'));
check("the DeepSeek key is server-only and the Gateway key is never a fallback", generation.includes('env.get("DEEPSEEK_API_KEY")') && generationIndex.includes("generationApiKey") && !generationIndex.includes("AI_GATEWAY_API_KEY") && !generation.includes("AI_GATEWAY_API_KEY") && !generationClient.includes("AI_GATEWAY_API_KEY"));
check("the DeepSeek call is bounded by max_tokens, thinking off and a deadline", generationClient.includes("GENERATION_BODY_LIMIT") && generation.includes("max_tokens") && generation.includes('thinking: { type: "disabled" }') && generationClient.includes("AbortSignal.timeout"));
...
check("verified DeepSeek pricing carries traceable official provenance", generationPricing.includes("deepseek-flash") && generationPricing.includes("deepseek-official-pricing") && generationPricing.includes("api-docs.deepseek.com/quick_start/pricing") && generationPricing.includes("pricingTier: \"peak\"") && generationPricing.includes("retention"));
check("the budget uses conservative peak cache-miss input and output rates", generationPricing.includes("inputPer1kUsd: 0.0003") && generationPricing.includes("outputPer1kUsd: 0.0012") && generationPricing.includes("cachedInputPer1kUsd: 0.000006"));
check("the usage bound comes from reported tokens and the billed cost is never invented", generation.includes("usageCostUpperBoundUsd") && generationClient.includes("prompt_cache_hit_tokens") && generationClient.includes("usageCostUpperBoundUsd") && generationClient.includes("cost: null"));
...
check("known DeepSeek failures map to fixed codes by status, never loose message-token matching", generationClient.includes("classifyProviderFailure") && generationClient.includes("status !== 402") && generationClient.includes("generation_insufficient_balance") && !/message.*\.(match|test)\(/.test(generationClient));
...
check("primary generation failure copy is friendly and code-free; diagnostics stay on the saved job", clientDataAssets.includes("generationDiagnosticSchema") && clientDataAssets.includes("generationFailureReason") && clientDataAssets.includes("generationJobDiagnostic") && clientDataAssets.includes("DeepSeek account balance is insufficient") && clientDataAssets.includes("previous Vercel AI Gateway provider") && clientDataAssets.includes("do not need Gateway credits") && !clientDataAssets.includes("safe codes") && !/generationFailureReason[\s\S]*?parts\.push/.test(clientDataAssets) && generatePanel.includes("generationFailureReason") && !generatePanel.includes("data-da-generation-reason"));
check("a peak-rate usage bound is never stored or shown as the actual billed cost", handler.includes("usageCostUpperBoundUsd") && handler.includes("actualCostUsd: outcome.cost") && generation.includes("usageCostUpperBoundUsd") && generationClient.includes("usageCostUpperBoundUsd") && generationClient.includes("cost: null") && generatePanel.includes("Usage cost upper bound"));
