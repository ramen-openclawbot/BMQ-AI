...
def test_explicit_revenue_date_cron_window_and_metadata() -> None:
    for needle, label in [
        ...
        ("fetchCurrentKioskPointEvidence(supabaseAdmin, window)", "scheduled recovery checks current kiosk evidence"),
        ("fetchPostedKioskPointEvidence", "scheduled recovery compares posted kiosk evidence"),
        ("currentKioskEvidence.fingerprint !== postedKioskEvidence.fingerprint", "changed kiosk evidence bypasses stale-document skip"),
        ('recoveryReason: "kiosk_point_evidence_changed"', "changed kiosk evidence records refresh reason"),
