...
class DealerDuplicateOrderGuardTests(unittest.TestCase):
    ...
    def test_atomic_rpc_serializes_and_deduplicates_submissions(self) -> None:
        for marker in (
            ...
            self.assertIn(marker, self.base_migration)
        for marker in (
            "o.submitted_at >= p_submitted_at - interval '30 minutes'",
            "o.submitted_at <= p_submitted_at + interval '30 minutes'",
            "p_duplicate_action not in ('add', 'continue')",
            "p_duplicate_action is distinct from 'add'",
            "p_duplicate_action is distinct from 'continue'",
        ):
    ...
    def test_chat_renders_duplicate_choice_without_creating_order(self) -> None:
        ...
        self.assertIn("Quý khách hàng muốn tiếp tục hay huỷ?", self.portal)
        self.assertIn(
            ">Đơn hàng tương tự đã được đặt! Quý khách hàng muốn tiếp tục hay huỷ?</div>",
            self.portal,
        )
        self.assertIn("Mã đơn trước:", self.portal)
        self.assertIn('data-dealer-chat-choices="duplicate-order"', self.portal)
        self.assertIn('data-dealer-chat-choice="duplicate-continue"', self.portal)
        self.assertIn('data-dealer-chat-choice="duplicate-cancel"', self.portal)
        self.assertIn("Tiếp tục", self.portal)
        self.assertIn("Huỷ", self.portal)
        self.assertIn("client_submission_id", self.portal)
        self.assertIn('duplicateAction?: "continue"', self.portal)
        self.assertIn("duplicate_action", self.portal)
        self.assertIn('body.duplicate_action === "continue"', self.submit)
