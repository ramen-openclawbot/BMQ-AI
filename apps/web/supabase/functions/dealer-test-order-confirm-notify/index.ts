...
const featureEnabled = async (supabase: ReturnType<typeof createServiceClient>) => {
  const { data, error } = await supabase
    .from("app_settings")
    .select("value")
    .eq("key", "dealer_test_order_confirmation_enabled")
    .maybeSingle();
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("[dealer-test-order-confirm-notify] Feature setting unavailable", error instanceof Error ? error.message : error);
  ...
  const { data, error } = await supabase.rpc("claim_dealer_test_order_confirmations", {
    batch_size: batchSize,
  });
  if (error) {
    console.error("[dealer-test-order-confirm-notify] Claim failed", error.message);
  ...
  for (const job of jobs) {
    const { data: contact, error: contactError } = await supabase
      ...
      .select("phone_normalized, is_test")
    ...
    if (contactError) {
      const retryExhausted = job.attempt_count >= job.max_attempts;
      const { error: requeueError } = await supabase
        .from("dealer_test_order_confirmations")
        .update({
      ...
      if (requeueError) console.error("[dealer-test-order-confirm-notify] Contact lookup requeue failed", requeueError.message);
    ...
    if (!contact?.phone_normalized) {
      const { error: suppressError } = await supabase
        ...
        .eq("id", job.id)
        .eq("status", "processing");
      if (suppressError) console.error("[dealer-test-order-confirm-notify] Could not suppress missing contact", suppressError.message);
      suppressed += 1;
      continue;
    }
    ...
    try {
      ...
      const { data: sendCommitted, error: commitError } = await supabase.rpc(
        "commit_dealer_test_order_confirmation_send",
      ...
      if (commitError) {
        ...
        const { error: requeueError } = await supabase
          ...
          .eq("id", job.id)
          .eq("status", "processing");
        if (requeueError) console.error("[dealer-test-order-confirm-notify] Send-commit requeue failed", requeueError.message);
        if (retryExhausted) failed += 1;
        else retried += 1;
        continue;
      }
      if (sendCommitted !== true) {
        const { error: suppressError } = await supabase
          ...
          .eq("id", job.id)
          .eq("status", "processing");
        if (suppressError) console.error("[dealer-test-order-confirm-notify] Send-commit suppression failed", suppressError.message);
        suppressed += 1;
        continue;
      }
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        console.error("[dealer-test-order-confirm-notify] Finalize retry failed", finalizeError?.message || "row_not_updated");
      }
      if (finalized) sent += 1;
      else console.error(`[dealer-test-order-confirm-notify] Provider accepted but finalization is pending for ${job.id}`);
    } catch (sendError) {
      ...
      if (updateError) console.error("[dealer-test-order-confirm-notify] Terminal failure status update failed", updateError.message);
      failed += 1;
    }
  }
