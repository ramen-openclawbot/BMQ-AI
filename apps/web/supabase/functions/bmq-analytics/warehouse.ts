...
export async function runWarehouse(raw: unknown, call: WarehouseCall, model: ModelCall, signal: AbortSignal, liveQuery?: Dependencies["query"]) {
  ...
  const response = (answer: string, lane: string, queries: unknown[] = [], citations: unknown[] = []) => ({ answer, requestId, provenance: {lane,model:modelCalls?MODEL:null,queries,citations,evidence,elapsedMs:Date.now()-started,modelCalls,usage,semanticVersion:"warehouse-bmq-v2"} });
  ...
  // Preserve the existing exact fast path and its RLS query even if the SSD is offline.
  const existingFast = liveQuery && !input.history.length ? fastQuery(input.question) : null;
  if (existingFast && liveQuery) {
  ...
  if (liveQuery) {
    // The source-doc approval join and payables tables are not in raw sync.
    // Keep these contracts live instead of reinterpreting raw rows as revenue.
    catalog.metrics = {...catalog.metrics,...METRICS};
    catalog.dimensions = {...catalog.dimensions,channel:"Revenue channel",status:"Purchase order status",category:"Inventory category",payment_method:"Payment method"};
  }
  ...
  const fastMetric = /^(doanh thu hôm nay|revenue today)$/.test(text)?"revenue":/^(số đơn đại lý hôm nay|dealer order count today)$/.test(text)?"dealer_order_count":/^(giá trị đơn đại lý hôm nay|dealer ordered value today)$/.test(text)?"dealer_order_value":/^(số báo cáo điểm bán hôm nay|kiosk report count today)$/.test(text)?"kiosk_report_count":/^(số đơn hôm nay|order count today)$/.test(text)?"order_count":null;
  ...
  for(const q of plan.queries) {
    ...
    if (liveQuery && Object.hasOwn(METRICS,q.metric)) {
