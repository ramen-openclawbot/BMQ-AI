-- Roll each point of sale's last submitted closing inventory into the next report opening.
-- Persist the source report for an auditable inventory chain; existing submitted history is untouched.

alter table public.kiosk_daily_reports
  add column if not exists opening_source_report_id uuid references public.kiosk_daily_reports(id) on delete restrict;

alter table public.kiosk_daily_reports
  add column if not exists opening_source_report_date date;

alter table public.kiosk_daily_reports
  drop constraint if exists kiosk_daily_reports_opening_source_before_report_date;

alter table public.kiosk_daily_reports
  add constraint kiosk_daily_reports_opening_source_before_report_date
  check (opening_source_report_date is null or opening_source_report_date < report_date);
