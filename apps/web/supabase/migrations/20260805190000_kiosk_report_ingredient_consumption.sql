-- Derive ingredient usage from finished Bánh mì que sales.
-- One Pate box prepares 20 Bánh mì que; ingredients cannot be sold at retail.

alter table public.kiosk_report_products
  add column if not exists sale_allowed boolean not null default true;

alter table public.kiosk_report_products
  add column if not exists breadstick_consumption_ratio numeric(12,6) not null default 0
  check (breadstick_consumption_ratio >= 0);

update public.kiosk_report_products
set sale_allowed = false, breadstick_consumption_ratio = 0.05
where code = 'pate';

update public.kiosk_report_products
set sale_allowed = false, breadstick_consumption_ratio = 0
where code = 'ot';

update public.kiosk_report_products
set sale_allowed = true, breadstick_consumption_ratio = 0
where code in ('banh_mi_que', 'banh_mi_say');

alter table public.kiosk_daily_report_inventory_rows
  drop column if exists closing_quantity;
...
alter table public.kiosk_daily_report_inventory_rows
  add column closing_quantity numeric(12,3) generated always as
    (opening_quantity + received_quantity - shortage_quantity + transfer_quantity - waste_quantity - returns_quantity - sold_quantity - consumed_quantity) stored;

-- Draft reports may contain legacy ingredient sales. Normalize only mutable drafts;
-- submitted reports remain an immutable historical record.
update public.kiosk_daily_report_inventory_rows inventory
set sold_quantity = 0
from public.kiosk_daily_reports report,
     public.kiosk_report_products product
where inventory.report_id = report.id
  and inventory.product_code = product.code
  and report.status = 'draft'
  and product.sale_allowed = false
...
begin
  ...
  select
    ...
    round(v_breadstick_sold * product.breadstick_consumption_ratio, 3),
