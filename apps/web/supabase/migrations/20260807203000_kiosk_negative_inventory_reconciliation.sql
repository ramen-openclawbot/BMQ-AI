-- Reconcile impossible negative prior closings with a physical opening count.
-- Drafts may retain warnings, but submitted reports must never close below zero.

alter table public.kiosk_daily_report_inventory_rows
  add column if not exists opening_reconciliation_required boolean not null default false;
...
declare
  ...
  v_existing_opening_reconciliations jsonb := '{}'::jsonb;
...
begin
  ...
  if v_report_id is null then
    ...
    select
      coalesce(jsonb_object_agg(inventory.product_code, inventory.opening_quantity), '{}'::jsonb),
      coalesce(jsonb_object_agg(inventory.product_code, inventory.opening_reconciliation_required), '{}'::jsonb)
      into v_existing_openings, v_existing_opening_reconciliations
  ...
  select
    ...
    case
      when v_use_previous_closing and coalesce(previous_inventory.closing_quantity < 0, false) then
      ...
      when not v_is_new_report
        and coalesce((v_existing_opening_reconciliations ->> product.code)::boolean, false) then
        greatest(0, coalesce(nullif(input.row_data->>'opening_quantity', '')::numeric, 0))
    ...
    case
      when v_use_previous_closing then coalesce(previous_inventory.closing_quantity < 0, false)
      when not v_is_new_report then coalesce((v_existing_opening_reconciliations ->> product.code)::boolean, false)
      else false
    end,
  ...
  if p_status = 'submitted' and exists (
    ...
    from public.kiosk_daily_report_inventory_rows inventory
    where inventory.report_id = v_report_id
      and inventory.closing_quantity < 0
  ) then
    raise exception 'negative_closing_inventory';
  end if;
