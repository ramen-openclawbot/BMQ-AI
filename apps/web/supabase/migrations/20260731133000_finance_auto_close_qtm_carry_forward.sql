-- Make QTM carry-forward system-owned: each approved closing balance becomes
-- the next day's authoritative opening balance. CEO input is never blocked for
-- a stale stored opening; UNC/QTM Drive evidence and chronological close remain gates.

update public.finance_daily_close_runs
set status = 'failed',
    decision = 'block',
    result = coalesce(result, '{}'::jsonb) || jsonb_build_object(
      'invalidatedBy', '20260731133000_finance_auto_close_qtm_carry_forward',
      'reason', 'qtm_opening_is_system_derived'
    ),
    finished_at = coalesce(finished_at, now()),
    updated_at = now(),
    updated_by = 'migration_20260731133000'
where mode = 'shadow'
...
declare
  ...
  v_effective_snapshot jsonb := coalesce(p_snapshot, '{}'::jsonb);
  v_audit_entry jsonb;
begin
  ...
  if found then
    if coalesce(v_previous_declaration.extraction_meta->>'close_approval_locked', 'false') = 'true' then
      v_previous_closing := coalesce(
        nullif(v_previous_declaration.extraction_meta->>'qtm_closing_balance', '')::numeric,
        coalesce(nullif(v_previous_declaration.extraction_meta->>'qtm_opening_balance', '')::numeric, 0)
          + coalesce(v_previous_declaration.qtm_extracted_amount, v_previous_declaration.cash_fund_topup_amount, 0)
          - coalesce(nullif(v_previous_declaration.extraction_meta->>'qtm_spent_from_folder', '')::numeric, 0)
      );
      -- System-owned carry-forward: the previous approved close is authoritative.
      v_qtm_opening := v_previous_closing;
      v_qtm_closing := v_qtm_opening + v_qtm_topup - v_qtm_spent;
      v_effective_snapshot := jsonb_set(
        jsonb_set(v_effective_snapshot, '{qtmOpening}', to_jsonb(v_qtm_opening), true),
        '{qtmClosing}',
        to_jsonb(v_qtm_closing),
        true
      ) || jsonb_build_object(
        'qtmOpeningSourceDate', v_previous_declaration.closing_date
      );
    end if;
  elsif v_qtm_opening <> 0 then
    v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
      'code', 'qtm_initial_opening_mismatch',
  ...
  set extraction_meta = jsonb_set(
        coalesce(v_declaration.extraction_meta, '{}'::jsonb)
          || jsonb_build_object(
            ...
            'qtm_opening_source_date', v_previous_declaration.closing_date,
  ...
  -- If tomorrow's declaration already exists and is still open, persist the
  -- authoritative carry-forward immediately. If it does not exist yet, the
  -- snapshot/UI derives the same opening from this approved close.
  update public.ceo_daily_closing_declarations
  set extraction_meta = coalesce(extraction_meta, '{}'::jsonb)
        || jsonb_build_object(
          'qtm_opening_balance', v_qtm_closing,
          'qtm_opening_source_date', p_closing_date,
          'qtm_opening_carried_at', now()
        ),
      updated_at = now()
  where closing_date = p_closing_date + 1
    and coalesce(extraction_meta->>'close_approval_locked', 'false') <> 'true'
    and 1 = (
      select count(*)
      from public.ceo_daily_closing_declarations next_day
      where next_day.closing_date = p_closing_date + 1
    );
