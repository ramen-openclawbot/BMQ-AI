-- Require every earlier finance declaration to be closed before a later day can auto-close.
-- Preserve immediate-previous-day QTM balance-chain validation separately.
...
update public.finance_daily_close_runs as r
set status = 'failed',
    decision = 'block',
    blockers = coalesce(r.blockers, '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
      'code', 'stale_shadow_invalidated',
      'invalidatedBy', '20260731104500_full_prior_chain'
    )),
    blocker_count = r.blocker_count + 1,
    result = coalesce(r.result, '{}'::jsonb) || jsonb_build_object(
      'invalidatedBy', '20260731104500_full_prior_chain'
    ),
    updated_at = now(),
    updated_by = 'system_finance_migration'
where r.mode = 'shadow'
  and r.status = 'succeeded'
  and exists (
    select 1
    from public.ceo_daily_closing_declarations as d
