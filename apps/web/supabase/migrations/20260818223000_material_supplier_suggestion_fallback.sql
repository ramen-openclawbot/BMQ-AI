-- Suggestion fallback remains read-only: broad deterministic name evidence is
-- displayed for human review but never auto-linked. Authorized editors may also
-- explicitly confirm a supplier/name/unit identity when no historical row exists.
      ...
      row_number() over (
        ...
        order by e.confirmed desc,
          case e.candidate_source
            ...
            when 'payment_history_name_unit' then 4
            when 'payment_history_name_contains' then 5
            else 6
          end,
      ...
      insert into public.material_supplier_products (
        ...
        jsonb_build_object(
          ...
          'selection_source', case
            when p_product_sku_id is not null then 'product_skus'
            when v_evidence_count > 0 then 'payment_history'
            else 'manual_supplier_selection'
          end,
