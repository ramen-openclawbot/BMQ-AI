-- Cron auto-posts one Vietnam business day of PO/email parse revenue as temporary controlled revenue.
-- This intentionally bypasses owner approval while preserving review flags for later ledger/audit work.

create unique index if not exists uq_revenue_source_documents_active_auto_daily_po_parse
  on public.revenue_source_documents((summary->>'auto_daily_no_double_count_key'))
  where status = 'controlled'
    and source_type = 'po_email_parse'
    and summary->>'monthly_parse_kind' = 'auto_daily_post'
    and summary->>'auto_daily_no_double_count_key' is not null;
...
begin
  ...
  if _run.po_received_from > _run.revenue_date_from or _run.po_received_to < _run.revenue_date_from then
    raise exception 'Auto daily PO received window must cover the revenue date';
  end if;
