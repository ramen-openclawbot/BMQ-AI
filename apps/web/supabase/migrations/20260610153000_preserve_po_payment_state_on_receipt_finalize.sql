...
BEGIN
  ...
  -- Preserve paid/approved PO advance request state. The receipt queue may still
  -- point at a PO-time Duyệt chi row; finalization must not reset that row back
  -- to pending/unpaid receipt-payable semantics. Reuse eligible receipt payable rows only
  -- rows only, otherwise create a separate Công nợ nhập kho request below.
