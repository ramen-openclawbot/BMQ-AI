...
export type AnalyticsMessage = { id: string; role: "user" | "assistant"; text: string; images?: UncImage[]; citations?: AnalyticsCitation[]; details?: string; fx?: { vndPerUsd: number; updatedAt: string; source: string } | null; customerSelection?: unknown; costContext?: unknown };
const analyticsResponse = z.object({
  answer: z.string().trim().min(1), requestId: z.string().min(1),
  provenance: z.object({ images:z.array(uncImageSchema).max(8).optional(), details: z.string().max(100000).optional(), fx: z.object({vndPerUsd:z.number().finite().positive(), updatedAt:z.string(), source:z.literal("https://www.exchangerate-api.com")}).nullable().optional(), customerSelection: z.unknown().optional(), costContext: z.unknown().optional(), lane: z.string(), model: z.string().nullable(), queries: z.array(z.unknown()), citations: z.array(z.object({ id: z.string(), title: z.string(), source: z.string(), updated_at: z.string() })).optional(), elapsedMs: z.number().finite().nonnegative() }),
});
...
export function parseAnalyticsResponse(data: unknown, language: "en" | "vi" = "vi") {
  ...
  return { ...result.data, provenance: { ...result.data.provenance, fx: result.data.provenance.fx as AnalyticsMessage["fx"], citations: result.data.provenance.citations as AnalyticsCitation[] | undefined } };
}
