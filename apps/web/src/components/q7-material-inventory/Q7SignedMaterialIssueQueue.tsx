              ...
              {q7SignedMaterialIssues.map((issue) => {
                ...
                return (
                  <article key={issue.id} data-testid={`q7-signed-material-issue-card-${issue.id}`} className="min-w-0 rounded-2xl border border-border bg-muted/40 p-4">
                    <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-muted-foreground">Số phiếu</p><p className="mt-1 break-words font-mono text-base font-bold text-primary">{issue.issue_number}</p></div><Badge variant="outline" className="shrink-0 whitespace-nowrap border-primary/25 bg-primary/5 text-primary">{q7SignedMaterialIssueStatusLabels[issue.status] || issue.status}</Badge></div>
