  ...
  if (isError) {
    return (
      <div className="rounded-2xl border border-red-100 bg-white p-5 shadow-sm">
        <div className="space-y-3">
          <p className="font-semibold text-red-700">{pc.unableToLoadSuppliers}</p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm" data-bmq-suppliers-single-list-header>
        <div className="border-b border-slate-100 p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-950">{pc.supplierList}</p>
              <p className="text-xs text-slate-500">{filteredSuppliers.length}/{suppliers?.length || 0}  {pc.suppliers2}</p>
            </div>
            <div className="relative w-full sm:max-w-sm">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <Input
                placeholder={pc.searchNameCategoryPhoneEmail}
                value={searchTerm}
        ...
        {!suppliers?.length ? (
          <div className="flex flex-col items-center justify-center px-4 py-10 text-center">
            ...
            <p className="font-semibold text-slate-900">{pc.noSuppliersYet}</p>
            <p className="mt-1 text-sm text-slate-500">{pc.addYourFirstSupplierToManagePurchasingAnd}</p>
          </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500">{supplier.category || pc.uncategorized}</p>
                    ...
                    <div className="mt-2 flex flex-col gap-1 text-xs text-slate-500 sm:flex-row sm:flex-wrap sm:gap-x-4">
                      ...
                      {!supplier.phone && !supplier.email && <span>{pc.noContactInformation}</span>}
                    </div>
                  </div>
                </div>
              </button>
          ...
          <p className="px-4 py-8 text-center text-sm text-slate-500">{pc.noMatchingSuppliers}</p>
        )}
      </div>
