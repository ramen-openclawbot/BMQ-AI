...
export function KnowledgeBaseProfileEditor(props: Props) {
  ...
  const activeTemplateName = poTemplates.find((t: any) => t.customer_id === editingCustomerId)?.file_name || f("Chưa có");
  ...
  const lockDisabledReason = !hasPassingEvidence ? f("Cần ít nhất 1 evidence pass trước khi lock contract.") : "";
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
            <Brain className="h-4 w-4 text-primary" />
             {f("Cấu hình parse PO")} </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
             {f("Mục tiêu: test trên PO thật, lưu evidence, khóa contract và tạo version KB thật trước khi đưa sang flow sync → doanh thu.")} </p>
        </div>
        <div className="rounded-full border border-border/70 bg-background px-3 py-1.5 text-xs text-muted-foreground">
           {f("Pending:")} <span className="font-semibold text-foreground">{pendingCount}</span>
        </div>
      </div>
      ...
      {!!kbAiStatus && (
        <div className={`rounded-xl border px-4 py-3 text-sm ${successTone ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : errorTone ? "border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-300" : "border-border/60 bg-background/80 text-muted-foreground"}`}>
          <div className="flex items-center gap-2">
            ...
            <span>{kbAiStatusText ?? kbAiStatus}</span>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-border/70 bg-background/80 p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            <div className="text-sm font-semibold text-foreground">{f("KB đang áp dụng")}</div>
            {activeKnowledgeProfile ? (
              <>
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 font-medium text-emerald-700 dark:text-emerald-300">{f("ACTIVE")}</span>
                  {latestVersion ? <span className="rounded-full border border-border/70 bg-muted/30 px-2.5 py-1">{f("KB v")}{latestVersion.version_no}</span> : null}
                  {contractStatus === "locked" && (
                    <span className="flex items-center gap-1 rounded-full border border-blue-500/30 bg-blue-500/10 px-2.5 py-1 font-medium text-blue-700 dark:text-blue-300">
                      <Lock className="h-3 w-3" />  {f("Locked")} </span>
                  )}
                  {contractStatus === "draft" && (
                    <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 font-medium text-amber-700 dark:text-amber-300">{f("Draft")}</span>
                  )}
                </div>
                <div className="text-lg font-semibold text-foreground">{activeKnowledgeProfile.profile_name || f("KB hiện tại")}</div>
                <div className="grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
                  <div>{f("PO mode:")} <span className="font-medium text-foreground">{activeKnowledgeProfile.po_mode || "-"}</span></div>
                  <div>{f("Template:")} <span className="font-medium text-foreground">{activeTemplateName}</span></div>
                  <div>{f("Evidence:")} <span className="font-medium text-foreground">{parseContract?.test_evidence?.length || 0}</span></div>
                </div>
              </>
            ) : (
              <div className="rounded-lg border border-dashed border-border/70 bg-background/70 px-4 py-5 text-sm text-muted-foreground">
                 {f("Khách hàng này chưa có KB active. Mở form bên dưới để tạo KB đầu tiên.")} </div>
            )}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" onClick={() => setHistoryOpen((v) => !v)}>
              <History className="mr-1 h-4 w-4" />
              {historyOpen ? f("Ẩn lịch sử") : f("Lịch sử KB{count}", { count: visibleHistory.length ? ` (${visibleHistory.length})` : "" })}
            </Button>
            <Button type="button" variant="outline" onClick={() => setEditorOpen((v) => !v)}>
              ...
              {editorOpen ? f("Ẩn cấu hình") : activeKnowledgeProfile ? f("Chỉnh sửa cấu hình") : f("Tạo cấu hình")}
            </Button>
          </div>
        </div>
      </div>
              ...
              return (
                <div key={item.id || `${item.customer_id}-${item.version_no}`} className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2 text-sm">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-foreground">{f("KB v")}{item.version_no}</span>
                    <span className={`text-xs ${isCurrent ? "text-emerald-700 dark:text-emerald-300" : "text-muted-foreground"}`}>{isCurrent ? f("CURRENT") : f("OLD")}</span>
                  </div>
        ...
        <div className="space-y-4 rounded-xl border border-border/70 bg-muted/10 p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
            <FileText className="h-4 w-4 text-primary" />
             {f("Tạo version mới")} </div>

          <div className={sectionClass}>
            <div className="mb-3 flex items-center gap-2 text-sm font-medium text-foreground">
              <FileText className="h-4 w-4 text-primary" />
               {f("Bước 1 · Cấu hình parse")} </div>
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("Tên profile")}</Label>
                  <Input value={editKbProfileName} onChange={(e) => onKbProfileNameChange(e.target.value)} placeholder={f("Ví dụ: Đại lý cấp 1 - Anh Thanh Knowledge")} disabled={isContractLocked} />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("Template tham chiếu")}</Label>
              ...
              <div className="grid gap-3 md:grid-cols-1">
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("PO mode")}</Label>
              ...
              <div className="space-y-2">
                <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("Mô tả business")}</Label>
                <textarea
                  className={inputClass}
                  value={editKbBusinessDescription}
                  onChange={(e) => onKbBusinessDescriptionChange(e.target.value)}
                  placeholder={f("Ví dụ: mỗi dòng là một điểm bán, tên điểm nằm trước dấu :, ưu tiên qty_total để sản xuất")}
                  disabled={isContractLocked}
                />
              </div>

              <div className="space-y-2">
                <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("Template / mẫu PO")}</Label>
                <div className="flex flex-col gap-2 lg:flex-row">
                  ...
                  <Button type="button" variant="outline" className="shrink-0" onClick={onClearTemplate} disabled={isContractLocked}>{f("Xoá mẫu")}</Button>
                </div>
                {templateFileName && (
                  <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-xs text-emerald-700 dark:text-emerald-300">
                     {f("Đã chọn mẫu mới:")} <b>{templateFileName}</b>
                  </div>
                )}
                {templateAiContext && (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                       {f("Chỉ dùng để tạo draft suggestion. Quyết định cuối phải dựa trên test PO thật.")} </p>
                  </div>
                  <div className="flex gap-2">
                    <Button type="button" variant="outline" onClick={() => setAiOpen((v) => !v)}>
                      {aiOpen ? f("Ẩn gợi ý") : f("Xem gợi ý")}
                    </Button>
                  ...
                  kbAiSuggestion ? (
                    <div className="space-y-3 rounded-lg border border-border/60 bg-muted/20 p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="text-sm font-medium text-foreground">{f("Đề xuất AI hiện tại")}</div>
                        <div className="rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                           {f("Confidence:")} <span className="font-semibold text-foreground">{Math.round(Number(kbAiSuggestion.confidence || 0) * 100)}%</span>
                        </div>
                      </div>
          ...
          <div className={sectionClass}>
            <div className="mb-3 flex items-center gap-2 text-sm font-medium text-foreground">
              <Sparkles className="h-4 w-4 text-primary" />
               {f("Bước 2 · Kiểm tra parse trên PO đã sync")} </div>
            <ParseTestWorkbench
          ...
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4 space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-foreground">{f("Bước 3 · Review & tạo version")}</div>
                <div className="mt-1 text-xs text-muted-foreground">
                  {isContractLocked
                    ? f("Contract đã khóa{time}. Sau khi tạo version thành công, hãy dùng flow sync → doanh thu để chạy hàng loạt.", { time: parseContract?.locked_at ? ` • ${new Date(parseContract.locked_at).toLocaleString("vi-VN")}` : "" })
                    : f("Contract đang ở draft. Cần ít nhất 1 evidence pass trước khi khóa contract và tạo version KB mới.")}
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {isContractLocked ? (
                  <Button type="button" variant="outline" onClick={handleUnlockContract}>
                    <LockOpen className="mr-1 h-4 w-4" />  {f("Mở khóa")} </Button>
                ) : (
                  <Button type="button" variant="outline" onClick={handleLockContract} disabled={!canLock}>
                    <Lock className="mr-1 h-4 w-4" />  {f("Khóa contract")} </Button>
                )}
              </div>
            </div>
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
              <div className="space-y-2">
                <Label className="text-xs uppercase tracking-wide text-muted-foreground">{f("Change note")}</Label>
                <Input value={kbChangeNote} onChange={(e) => onKbChangeNoteChange(e.target.value)} placeholder={f("Mô tả thay đổi để tạo version KB mới")} />
              </div>
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="secondary" onClick={onApproveLatest} disabled={approvePending || !canApproveLatest}>
                  ...
                  {approvePending ? f("Đang tạo version...") : f("Tạo version mới")}
                </Button>
              </div>
            </div>
            {!!approveDisabledReason && (
