...
const navItems = [
  ...
  { to: "/sku-costs/analysis", label: "cogs_trends", icon: BarChart3 },
  { to: "/sku-costs/management", label: "sku_management", icon: Package2 },
] as const;
...
function validateImageFile(file: File | null) {
  if (!file) return "please_select_an_sku_image" as const;
  if (!ALLOWED_IMAGE_TYPES.includes(file.type)) return "only_jpg_jpeg_png_or_webp_are_supported" as const;
  if (file.size > MAX_IMAGE_SIZE) return "maximum_image_size_is_5mb" as const;
  return null;
}
...
function SkuImageThumb({ sku, uploading, onClick }: { sku: SkuRow; uploading?: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      aria-label={formatText(s.update_image_for, { name: sku.product_name })}
    >
      {sku.image_url ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-1 bg-muted text-muted-foreground">
          ...
          <span className="text-[9px] font-bold leading-none">{s.no_image}</span>
        </div>
      )}
      <span className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-foreground/70 py-1 text-[9px] font-extrabold text-primary opacity-0 backdrop-blur transition group-hover:opacity-100">
        <Camera className="h-3 w-3" /> {s.change_image} </span>
      {uploading && (
...
export default function SkuCostsDjango() {
  ...
  const [uploadError, setUploadError] = useState<keyof typeof s | null>(null);
  ...
  const exportSkuSheet = () => {
    if (!filteredSkus.length) {
      toast.info(s.no_skus_to_export);
      return;
    }
    ...
    toast.success(s.sku_sheet_exported, { description: formatText(s.exported_sku_count, { count: filteredSkus.length }) });
  };
  ...
  const saveSkuImage = async () => {
    ...
    try {
      ...
      toast.success(s.sku_image_updated);
      setEditingSku(null);
      setSelectedFile(null);
    } catch (error: unknown) {
  ...
  return (
    <div data-i18n-batch="staff-sku-a-v1" data-stitch-sku-cost-dashboard-theme="pantone-2026-light" className="-m-4 min-h-screen bg-background text-foreground md:-m-6">
      <div className="mx-auto min-h-screen w-full max-w-[430px] bg-background px-4 pb-28 pt-3 shadow-2xl md:max-w-[560px] md:px-5 lg:hidden">
        <header className="sticky top-0 z-20 -mx-4 border-b border-border/60 bg-background/92 px-4 pb-3 pt-2 backdrop-blur-xl md:-mx-5 md:px-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="min-w-0 text-center">
              <h1 className="text-[21px] font-black leading-tight tracking-[-0.03em] text-foreground">{s.cogs_overview}</h1>
              <p className="mt-0.5 text-[11px] font-bold text-muted-foreground">{s.all_current_finished_skus}</p>
            </div>
          ...
          <nav className="mt-4 flex gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={s.cogs_navigation}>
            {navItems.map((item) => {
        ...
        <main className="space-y-4 pt-4">
          <section className="rounded-[28px] border border-border/70 bg-card/70 p-4 shadow-card backdrop-blur-xl">
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: s.finished_skus, value: loading ? "..." : stats.count, sub: s.under_management },
                { label: s.average_selling_price, value: loading ? "..." : vnd(stats.avgSelling), sub: formatText(s.priced_sku_count, { count: stats.pricedCount }) },
                { label: s.highest_price, value: loading ? "..." : vnd(stats.maxSelling), sub: s.highest_selling_price },
                { label: s.updated, value: loading ? "..." : formatDateTime(stats.updatedAt), sub: s.most_recent },
              ].map((card) => (
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {[
                { label: s.priced, value: stats.pricedCount, tone: "text-success" },
                { label: s.not_priced, value: stats.missingPrice, tone: stats.missingPrice ? "text-destructive" : "text-foreground/70" },
                { label: s.with_image, value: stats.imageCount, tone: "text-primary" },
              ].map((chip) => (
          ...
          <section className="rounded-[24px] border border-border/70 bg-card/80 p-4 shadow-card">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-[16px] font-black tracking-[-0.02em] text-foreground">{s.sku_price_distribution}</h2>
                <p className="mt-0.5 text-[11px] font-semibold text-muted-foreground">{s.a_quick_view_of_all_sku_price_bands}</p>
              </div>
          ...
          <section className="space-y-3">
            <div className="flex items-end justify-between gap-3">
              <div>
                <h2 className="text-[18px] font-black tracking-[-0.02em] text-foreground">{s.all_current_skus}</h2>
                <p className="mt-0.5 text-[11px] font-bold text-muted-foreground">{s.click_an_image_to_upload_or_update_it}</p>
              </div>
              <button
                type="button"
              ...
              >
                {sortMode === "price" ? s.selling_price : s.recently_updated}
              </button>
            </div>
            ...
            {loadFailed ? <p role="alert" className="text-sm text-destructive">{s.unable_to_load_skus} {s.please_try_again}</p> : loading ? (
              <div className="rounded-[24px] border border-border/70 bg-muted/40 px-4 py-8 text-center text-[13px] font-bold text-muted-foreground">{s.loading_skus}</div>
            ) : filteredSkus.length === 0 ? (
              <div className="rounded-[24px] border border-dashed border-border/70 bg-muted/35 px-4 py-8 text-center text-[13px] font-bold text-muted-foreground">{s.no_matching_skus_found}</div>
            ) : (
                ...
                {filteredSkus.map((sku) => {
                  ...
                  return (
                    <article key={sku.id} className="flex items-center gap-3 rounded-[24px] border border-border/70 bg-card/80 p-3 shadow-card">
                      ...
                      <button type="button" onClick={() => openSkuDetail(sku)} className="min-w-0 flex-1 text-left" aria-label={formatText(s.view_details_for, { name: sku.product_name })}>
                        ...
                        <div className="mt-3 flex items-center justify-between gap-2">
                          <div>
                            ...
                            <div className="mt-1 text-[10px] font-bold text-muted-foreground">{s.selling_price}</div>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <span className={cn("rounded-full px-2 py-1 text-[10px] font-extrabold ring-1", hasPrice ? "bg-success/10 text-success ring-success/20" : "bg-destructive/10 text-destructive ring-destructive/20")}>
                              {hasPrice ? s.priced_2 : s.not_priced_2}
                            </span>
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 mx-auto max-w-[430px] bg-gradient-to-t from-background via-background/96 to-transparent px-4 pb-4 pt-8 md:max-w-[560px] lg:hidden">
          <div className="grid grid-cols-2 gap-3 rounded-[24px] border border-border/70 bg-card/80 p-2 shadow-card backdrop-blur-xl">
            <Button onClick={exportSkuSheet} className="h-12 rounded-[18px] border border-border/70 bg-card/80 text-[13px] font-extrabold text-foreground hover:bg-muted/70">{s.export_sku_sheet}</Button>
            <Button onClick={() => navigate("/sku-costs/management")} className="h-12 rounded-[18px] bg-primary text-[13px] font-extrabold text-primary-foreground shadow-card hover:bg-primary/90">{s.update_prices}</Button>
          </div>
        </div>
      </div>
      ...
      <div className="hidden min-h-screen bg-background px-8 py-8 lg:block">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
          <header className="rounded-[32px] border border-border/70 bg-card/70 p-6 shadow-card backdrop-blur-xl">
            <div className="flex items-start justify-between gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-primary/70">{s.sku_costs}</p>
                  <h1 className="mt-1 text-[34px] font-black leading-tight tracking-[-0.04em] text-foreground">{s.cogs_overview}</h1>
                  <p className="mt-2 max-w-2xl text-sm font-semibold text-muted-foreground">{s.desktop_overview_of_all_finished_skus_selling_prices_costs_and_product_images}</p>
                </div>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <Button onClick={exportSkuSheet} className="h-11 rounded-2xl border border-border/70 bg-card/80 px-5 text-sm font-extrabold text-foreground hover:bg-muted/70">{s.export_sku_sheet}</Button>
                <Button onClick={() => navigate("/sku-costs/management")} className="h-11 rounded-2xl bg-primary px-5 text-sm font-extrabold text-primary-foreground shadow-card hover:bg-primary/90">{s.update_prices}</Button>
              </div>
            </div>
          ...
          <section className="grid grid-cols-4 gap-4">
            {[
              { label: s.finished_skus, value: loading ? "..." : stats.count, sub: s.under_management, tone: "text-foreground" },
              { label: s.average_selling_price, value: loading ? "..." : vnd(stats.avgSelling), sub: formatText(s.priced_sku_count, { count: stats.pricedCount }), tone: "text-primary" },
              { label: s.highest_price, value: loading ? "..." : vnd(stats.maxSelling), sub: s.highest_selling_price, tone: "text-warning" },
              { label: s.updated, value: loading ? "..." : formatDateTime(stats.updatedAt), sub: s.most_recent, tone: "text-success" },
            ].map((card) => (
          ...
          <section className="grid grid-cols-[minmax(0,0.95fr)_minmax(0,1.35fr)] gap-5">
            <article className="rounded-[30px] border border-border/70 bg-card/80 p-5 shadow-card">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-black tracking-[-0.03em] text-foreground">{s.sku_price_distribution}</h2>
                  <p className="mt-1 text-sm font-semibold text-muted-foreground">{s.price_bands_for_all_finished_skus}</p>
                </div>
            ...
            <article className="rounded-[30px] border border-border/70 bg-card/80 p-5 shadow-card">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-black tracking-[-0.03em] text-foreground">{s.data_status}</h2>
                  <p className="mt-1 text-sm font-semibold text-muted-foreground">{s.check_selling_prices_and_product_images_before_reviewing_cogs}</p>
                </div>
              ...
              <div className="mt-4 rounded-[24px] border border-warning/25 bg-warning/10 p-4 text-sm font-semibold leading-relaxed text-warning">
                {s.desktop_and_mobile_share_real_time_data_for_image_uploads_sheet_exports_and_sk}
              </div>
            </article>
          </section>
          ...
          <section className="rounded-[32px] border border-border/70 bg-card/80 p-5 shadow-card">
            <div className="mb-5 flex items-end justify-between gap-5">
              <div>
                <h2 className="text-2xl font-black tracking-[-0.03em] text-foreground">{s.all_current_skus}</h2>
                <p className="mt-1 text-sm font-bold text-muted-foreground">{s.click_an_image_to_upload_or_update_it_click_a_row_to_manage_the_sku}</p>
              </div>
              <div className="flex min-w-[420px] items-center gap-3">
                <div className="flex min-w-0 flex-1 items-center gap-2 rounded-2xl border border-border/70 bg-muted/45 px-3 py-2.5 shadow-inner">
                  ...
                  <Input
                    value={query}
                    onChange={(event) => setQuery(event.target.value)}
                    placeholder={s.search_sku_by_name_or_code}
                ...
                >
                  {s.sort} {sortMode === "price" ? s.selling_price : s.recently_updated}
                </button>
              </div>
            </div>

            {loadFailed ? <p role="alert" className="text-sm text-destructive">{s.unable_to_load_skus} {s.please_try_again}</p> : loading ? (
              <div className="rounded-[24px] border border-border/70 bg-muted/40 px-4 py-12 text-center text-sm font-bold text-muted-foreground">{s.loading_skus}</div>
            ) : filteredSkus.length === 0 ? (
              <div className="rounded-[24px] border border-dashed border-border/70 bg-muted/35 px-4 py-12 text-center text-sm font-bold text-muted-foreground">{s.no_matching_skus_found}</div>
            ) : (
                ...
                <table className="w-full border-collapse text-left">
                  <thead className="bg-muted/45 text-xs font-extrabold uppercase tracking-[0.12em] text-muted-foreground">
                    <tr>
                      ...
                      <th className="px-4 py-3">{s.selling_price}</th>
                      <th className="px-4 py-3">{s.cogs}</th>
                      <th className="px-4 py-3">LC %</th>
                      <th className="px-4 py-3">{s.status}</th>
                      <th className="px-4 py-3">{s.updated}</th>
                      <th className="px-4 py-3 text-right">{s.action}</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={sku.id} className="group bg-card/60 transition hover:bg-muted/45">
                          <td className="px-4 py-3">
                            <div className="flex min-w-0 items-center gap-3">
                              <SkuImageThumb sku={sku} uploading={uploadingSkuId === sku.id} onClick={() => openUploadModal(sku)} />
                              <button type="button" onClick={() => openSkuDetail(sku)} className="min-w-0 text-left" aria-label={formatText(s.view_details_for, { name: sku.product_name })}>
                          ...
                          <td className="px-4 py-3 text-right">
                            <Button type="button" onClick={() => openSkuDetail(sku)} className="h-10 rounded-2xl bg-muted/50 px-4 text-xs font-extrabold text-foreground hover:bg-primary/90 hover:text-primary-foreground">
                              {s.open_details} <ChevronRight className="ml-1.5 h-4 w-4" />
                            </Button>
                          </td>
                        </tr>
                      );
      ...
      <Dialog open={!!editingSku} onOpenChange={(open) => { if (!open && !uploadingSkuId) { setEditingSku(null); setSelectedFile(null); setUploadError(null); } }}>
        <DialogContent className="border-border/70 bg-card text-foreground sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle className="text-foreground">{s.update_sku_image}</DialogTitle>
          </DialogHeader>
          {editingSku && (
            <div className="space-y-4">
              ...
              <div className="overflow-hidden rounded-[22px] border border-border/70 bg-muted/45">
                {previewUrl || editingSku.image_url ? (
                  <img src={previewUrl || editingSku.image_url || ""} alt={s.sku_image_preview} className="h-56 w-full object-cover" />
                ) : (
                  <div className="flex h-56 flex-col items-center justify-center gap-2 text-muted-foreground">
                    <ImageIcon className="h-8 w-8" />
                    <span className="text-sm font-extrabold">{s.no_image}</span>
                  </div>
                )}
              </div>
              ...
              <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={!!uploadingSkuId} className="h-11 w-full rounded-2xl border-primary/25 bg-primary/10 text-primary hover:bg-primary/15 hover:text-primary">
                <Upload className="mr-2 h-4 w-4" /> {s.select_image} </Button>
              {selectedFile && <p className="text-xs font-bold text-muted-foreground">{s.selected} {selectedFile.name}</p>}
              {uploadError && (
          ...
          <DialogFooter className="gap-2 sm:gap-2">
            <Button type="button" variant="ghost" disabled={!!uploadingSkuId} onClick={() => { setEditingSku(null); setSelectedFile(null); setUploadError(null); }} className="text-muted-foreground hover:bg-muted/50 hover:text-foreground">{s.cancel}</Button>
