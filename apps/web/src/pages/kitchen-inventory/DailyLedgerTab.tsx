...
export function DailyLedgerTab({ periodMonth, items, movements, canEdit, addMovement }: DailyLedgerTabProps) {
  ...
  opening: c("Tồn đầu kỳ"),
  purchase: c("Nhập"),
  usage: c("Xuất dùng"),
  stock_count: c("Kiểm kê cuối"),
  adjustment: c("Điều chỉnh"),
};
  ...
  const [formError, setFormError] = useState<WarehouseCopyKey | null>(null);
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="rounded-xl border bg-card p-4 shadow-sm">
        <div className="mb-4">
          <h2 className="text-lg font-semibold">{c("Ledger hằng ngày")}</h2>
          <p className="text-sm text-muted-foreground">
            {c("Nhập theo form kế toán gốc, chọn item từ danh mục chuẩn để tự điền đơn vị và đơn giá.")} </p>
        </div>

        <div className="grid gap-3 md:grid-cols-6">
          <label className="space-y-1 md:col-span-1">
            <span className="text-sm font-medium">{c("Ngày")}</span>
            <input
              type="date"
              value={movementDate}
          ...
          <label className="space-y-1 md:col-span-1">
            <span className="text-sm font-medium">{c("Loại")}</span>
            <select
              value={movementType}
          ...
          <label className="space-y-1 md:col-span-2">
            <span className="text-sm font-medium">{c("Nguyên vật liệu")}</span>
            <select
              value={itemId}
              ...
              required
            >
              <option value="">{c("Chọn item chuẩn...")}</option>
          ...
          <label className="space-y-1">
            <span className="text-sm font-medium">{c("SL")}</span>
            <input
              type="number"
              step="0.001"
          ...
          <div className="space-y-1">
            <span className="text-sm font-medium">{c("Đơn giá chuẩn")}</span>
        ...
        <label className="mt-3 block space-y-1">
          <span className="text-sm font-medium">{c("Ghi chú")}</span>
          <input
            value={note}
            onChange={(event) => setNote(event.target.value)}
            placeholder={c("Bắt buộc khi điều chỉnh hoặc override sau này")}
            className="h-10 w-full rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring"
            disabled={!canEdit}
        ...
        <div className="mt-4 flex justify-end">
          <Button type="submit" disabled={!canEdit || addMovement.isPending || !selectedItem}>
            {addMovement.isPending ? c("Đang ghi...") : c("Ghi sổ")}
          </Button>
        </div>
      </form>
      ...
      <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
        <div className="border-b px-4 py-3">
          <h3 className="font-semibold">{c("Dòng ledger trong tháng")}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[780px] text-sm">
            <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                <th className="px-4 py-3">{c("Ngày")}</th>
                <th className="px-4 py-3">{c("Loại")}</th>
                <th className="px-4 py-3">{c("Item")}</th>
                <th className="px-4 py-3 text-right">{c("SL")}</th>
                <th className="px-4 py-3">{c("ĐVT")}</th>
                <th className="px-4 py-3 text-right">{c("Thành tiền")}</th>
                <th className="px-4 py-3">{c("Nguồn")}</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {movements.length === 0 && (
                <tr>
                  <td className="px-4 py-8 text-center text-muted-foreground" colSpan={7}>{c("Chưa có phát sinh trong tháng.")}</td>
                </tr>
              )}
