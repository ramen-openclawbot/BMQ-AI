...
const decisionLabel: Record<QueueDecision, string> = {
  auto_ready: "Khớp chính xác",
  review: "Cần xác nhận",
  blocked: "Chưa thể liên kết",
};

const sourceLabel = (row: ResolutionRequestRow) => row.source_table === "kitchen_inventory_items" ? "Phiếu xuất kho Q7" : row.source_table === "product_skus" ? "Giá vốn" : "Nguồn khác";
...
export default function ReconciliationQueue({ canMutate = false, sourceFilter = "all" }: { canMutate?: boolean; sourceFilter?: string }) {
  ...
  const linkMutation = useMutation({
    mutationFn: async (row: ResolutionRequestRow) => {
      if (!row.source_id || !row.resolved_material_id) throw new Error("Thiếu chứng từ nguồn hoặc NVL chuẩn đã duyệt.");
    ...
    onSuccess: () => {
      toast({ title: "Đã liên kết NVL chuẩn", description: "Tên và đơn vị chuẩn sẽ được dùng cho nghiệp vụ này." });
      queryClient.invalidateQueries({ queryKey: ["material-master", "material_resolution_requests", "task3"] });
    },
    onError: (mutationError: any) => {
      toast({ title: "Không thể liên kết", description: mutationError?.message || "Thông tin chưa đủ điều kiện để xác nhận.", variant: "destructive" });
    },
  });
    ...
    <div className="space-y-5 p-4 md:p-6" data-task3-reconciliation-queue>
      <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-amber-50 p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-emerald-700">Đồng bộ tên và đơn vị</p>
            <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Các dòng cần xác nhận</h1>
            <p className="mt-2 max-w-3xl text-sm text-slate-600">
              Kiểm tra tên đang dùng, tên NVL chuẩn và đơn vị trước khi liên kết với Giá vốn hoặc phiếu xuất kho Q7.
            </p>
          </div>
      ...
      <Alert className="border-amber-200 bg-amber-50">
        ...
        <AlertTitle className="text-amber-800">Không tự động ghép tên gần giống</AlertTitle>
        <AlertDescription className="text-amber-700">
          Tên gần giống chỉ là gợi ý. Anh cần kiểm tra đúng nguyên liệu và đơn vị trước khi xác nhận.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <CardDescription>So sánh tên và đơn vị đang dùng với danh mục NVL chuẩn.</CardDescription>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <Select value={decisionFilter} onValueChange={(value) => setDecisionFilter(value as QueueDecision | "all")}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="auto_ready">Khớp chính xác</SelectItem>
                  <SelectItem value="review">Cần xác nhận</SelectItem>
                  <SelectItem value="blocked">Chưa thể liên kết</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <CardContent className="space-y-3">
          {isLoading && <div className="flex items-center gap-2 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" /> Đang tải danh sách...</div>}
          {error && <div className="text-sm text-rose-600">Không tải được danh sách. Vui lòng thử lại.</div>}
              ...
              <div key={row.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">NVL chuẩn đề xuất: {row.material?.canonical_name || "Chưa có"} {row.material?.material_code ? `· ${row.material.material_code}` : ""}</p>
                    <p className="mt-1 text-xs text-slate-500">Nơi sử dụng: {sourceLabel(row)}</p>
                  </div>
                ...
                <div className="mt-4 grid gap-3 text-sm md:grid-cols-3">
                  <div className="rounded-xl bg-slate-50 p-3"><p className="font-medium text-slate-700">Cách nhận diện</p><p className="mt-1 text-slate-600">{evidence}</p></div>
