...
export default function StockReport() {
  ...
  const MOVEMENT_TYPE_LABELS: Record<string, { label: string; color: string }> =
    {
      goods_receipt_in: { label: c("Nhập NVL"), color: "bg-green-100 text-green-800" },
      production_consume: {
        label: c("Tiêu hao SX"),
        color: "bg-amber-100 text-amber-800",
      },
      production_output: { label: c("Nhập TP"), color: "bg-blue-100 text-blue-800" },
      dispatch_out: { label: c("Xuất kho"), color: "bg-red-100 text-red-800" },
      adjustment: { label: c("Điều chỉnh"), color: "bg-gray-100 text-gray-800" },
    };
  const now = new Date();
  ...
  return (
    <div data-bmq-warehouse-i18n="v1" className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <BarChart4 className="h-8 w-8 text-blue-600" />
            <h1 className="text-3xl font-bold text-gray-900">{c("Báo cáo tồn kho")}</h1>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
              ...
              <div>
                ...
                <Select value={skuTypeFilter} onValueChange={setSkuTypeFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">{c("Tất cả")}</SelectItem>
                    <SelectItem value="raw_material">{c("Nguyên vật liệu")}</SelectItem>
                    <SelectItem value="finished_good">{c("Thành phẩm")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {c("Tìm kiếm sản phẩm")} </label>
                <Input
                  placeholder={c("Nhập tên sản phẩm...")}
                  value={searchTerm}
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">{c("Tổng nhập kho")}</CardTitle>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-gray-600">
                    {c("Trong")} {format(new Date(dateFrom), "MMM yyyy", { locale: vi })}
                  </p>
                </>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">{c("Tổng xuất kho")}</CardTitle>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">{c("Tồn kho NVL")}</CardTitle>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-gray-600">{c("Nguyên vật liệu")}</p>
                </>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">{c("Tồn kho TP")}</CardTitle>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-gray-600">{c("Thành phẩm")}</p>
                </>
              )}
            </CardContent>
          </Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="py-8 text-center text-gray-500">
                {c("Không có dữ liệu tồn kho")} </div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      <TableHead className="font-semibold">{c("Tên sản phẩm")}</TableHead>
                      <TableHead className="font-semibold">{c("SKU Code")}</TableHead>
                      <TableHead className="font-semibold">{c("Loại")}</TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Tồn kho")} </TableHead>
                      <TableHead className="font-semibold">{c("Đơn vị")}</TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Tồn tối thiểu")} </TableHead>
                      <TableHead className="font-semibold">{c("Trạng thái")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {c("Lịch sử nhập xuất kho")} </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="flex items-center justify-center py-8">
              ...
              <div className="py-8 text-center text-gray-500">
                {c("Không có giao dịch trong khoảng thời gian này")} </div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      <TableHead className="font-semibold">{c("Ngày")}</TableHead>
                      <TableHead className="font-semibold">
                        {c("Loại giao dịch")} </TableHead>
                      <TableHead className="font-semibold">{c("Sản phẩm")}</TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Số lượng")} </TableHead>
                      <TableHead className="font-semibold">{c("Đơn vị")}</TableHead>
                      <TableHead className="font-semibold">
                        {c("Tham chiếu")} </TableHead>
                      <TableHead className="font-semibold">{c("Ghi chú")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="py-8 text-center text-gray-500">
                {c("Không có dữ liệu đối soát")} </div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      <TableHead className="font-semibold">{c("Sản phẩm")}</TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Tồn đầu kỳ")} </TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Nhập trong kỳ")} </TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Xuất trong kỳ")} </TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Tồn cuối kỳ (tính)")} </TableHead>
                      <TableHead className="text-right font-semibold">
                        {c("Tồn thực tế")} </TableHead>
                      <TableHead
                        className={`text-right font-semibold`}
                      >
                        {c("Chênh lệch")} </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
