...
Endpoints: `GET /v1/status`, `/v1/sources`, `/v1/semantic`; `POST /v1/ingest`, `/v1/documents`, `/v1/knowledge/search`, `/v1/query`, `/v1/customer`, `/v1/cost`. No public raw database, arbitrary SQL, raw customer download, deletion or training endpoint exists. Source list displays the latest 200 ingestion runs and latest 200 documents.

Read-only cost classification lane (`/v1/cost`, `bmq_cost.py`): seven closed questions over the reviewed raw snapshot (month totals by category/review status, pending review count/amount, largest pending lines with supplier/document, two-month category comparison, unmapped/low-confidence lines, one-line classification explanation with the stored rule/alias evidence, and sync freshness). It reproduces all four arms of `public.cost_classification_line_details` (classified payment request/invoice lines plus OCR-only payment request/invoice lines, never double counted): paid/created/invoice source dates cast from the UTC timestamp (never classification `created_at`), `coalesce(line_total, quantity*unit_price, 0)` amounts, source-line override of category/name/code/allocation/classification source, `UNMAPPED_REVIEW` forced to `needs_review`, invoice-linked or `invoice_created` payment requests excluded, and category labels from `cost_categories` only. Money is always the exact per-status sum, never a row-count ratio. Owner-only, fixed SQL, 30-minute freshness guard, and explicit provenance (`snapshot_id`, `source_observed_at`, `semantic_version`, `sync_version`); stale snapshots and unreconciled tables fail closed instead of returning zero. Numeric answers additionally require a snapshot manifest that declares the canonical cost projection columns (`COST_PROJECTION_FIELDS`, sync version `bmq-supabase-raw-v14`); an older or incomplete projection is refused rather than served as a subtly different total.
